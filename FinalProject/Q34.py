...
def lagrange(x, y, x_for_approx):
    ...
    for i in range(0, x.size):
        ...
        for j in range(0, x.size):
            ...
            num_of_iteration += 1
        print(
            f'f`(x) = {approx_result}+{poly}, iteration No.: {num_of_iteration}')
...
def neville(x, y, x_for_approx):
    ...
    coefficients_matrix = np.concatenate(
        (y[:, None], coefficients_matrix), axis=1)
...
if __name__ == '__main__':
    ...
    approx_result, coefficients_matrix = neville(
        x_values, y_values, x_for_approx)
